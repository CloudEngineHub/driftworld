    ...
    for i in range(seq_length):
        ...
        with sync_ctx:
            loss_i, gen_det, info_i = denoiser(
                prev_obs, prev_act, noise, target_obs, past_obs, b, k)
...
def train(cfg):
    ...
    if os.path.exists(cfg.path_ckpt_latest):
        ...
        missing, unexpected = inner.load_state_dict(ckpt['model'], strict=False)
        if is_main():
            log.info(f"Restored from phase 1's step {ckpt['step']} ckpt to begin phase 2 training")
            if missing or unexpected:
                log.info(f"phase-1 load (strict=False): {len(missing)} missing "
                         f"(kept at init), {len(unexpected)} unexpected")
        del ckpt
    barrier(world_size)
