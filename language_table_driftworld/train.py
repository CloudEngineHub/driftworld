"""
Training loop for DriftWorld on Language Table
"""
import os
...
from dataset.language_table_dataloader import get_language_table_dataloader
...
def train(cfg):
    """
    Train model (phase-1 only), given hydra config cfg. Multi-GPU via torchrun + DDP.
    """
    if cfg.model.is_phase_2:
        raise ValueError("train.py supports phase-1 training only. For phase-2 self-forcing, run train_selfforce.py instead.")
    ...
    if is_main():
        log.info("Creating dataloader for Language Table")
    dataloader = get_language_table_dataloader(
        cfg, split="train",
