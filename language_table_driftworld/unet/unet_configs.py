...
class InnerModelConfig:
    ...
    cond_channels: int # dim of conditioning vector
    ...
    attn_depths: List[bool] # whether self-attention should be applied at each specific resolution block
    num_actions: int = 2 # dim of action vector


def get_inner_model_config(config_name: str) -> InnerModelConfig:
    if config_name == "LT_UNet":
