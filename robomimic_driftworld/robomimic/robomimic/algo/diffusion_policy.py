...
class DiffusionPolicyUNet(PolicyAlgo):
    def _create_networks(self):
        ...
        if self.algo_config.ema.enabled:
            # diffusers>=0.13 EMAModel API: takes an iterable of parameters (not a model) and
            # tracks shadow copies. use_ema_warmup=True + inv_gamma=1.0 reproduces the decay
            # schedule of the old diffusers==0.11.1 EMAModel (which always used the warmup formula).
            ema = EMAModel(
    ...
    def _get_action_trajectory(self, obs_dict, goal_dict=None):
        ...
        # select network: temporarily swap in the EMA-averaged weights for inference, then
        # restore the raw training weights afterwards (diffusers>=0.13 EMAModel has no standalone
        # averaged_model module, so we copy its shadow params onto self.nets and restore).
        nets = self.nets
        if self.ema is not None:
            self.ema.store(nets.parameters())
