"""
Training loop for DriftWorld on Bridge-V2 -- Phase 2 self-forcing
"""
import os
...
from datasets.bridge_dataloader import get_bridge_dataloader
from encoders.vae_sd3 import VAE

log = logging.getLogger(__name__)


def create_selfforce_denoiser(cfg, device):
    from drifting_denoiser_selfforce import Denoiser as SelfForceDenoiser
    return SelfForceDenoiser(cfg).to(device)
...
def phase2_perstep_step(denoiser, inner, nbatch, device, world_size, cfg, optimizer, scheduler):
    """
    One optimizer step of phase-2 self-forcing.
    """
...
def train(cfg):
    ...
    if not cfg.model.is_phase_2:
        raise ValueError("train_selfforce.py supports phase-2 self-forcing only. For phase-1 training, run train.py instead.")
    ...
    dataloader = get_bridge_dataloader(
        cfg, split="train",
        rank=rank, world_size=world_size,
    )

    if is_main():
        log.info("Creating model (self-forcing per-step-backward variant)")
    denoiser = create_selfforce_denoiser(cfg, device)
    ...
    for epoch_idx in range(start_ep, cfg.train.num_epochs + 1):
        ...
        for batch_idx, nbatch in enumerate(dataloader):
            ...
            if batch_idx == 0 and epoch_idx == 1 and is_main():
                vid_key = 'video_latents' if 'video_latents' in nbatch else 'video'
                log.info(f"action: {nbatch['action'].shape} | min {nbatch['action'].min():.4f} | max {nbatch['action'].max():.4f}")
                log.info(f"{vid_key}: {nbatch[vid_key].shape} | min {nbatch[vid_key].min():.4f} | max {nbatch[vid_key].max():.4f}")

            # Phase-2 self-forcing with per-step backward (backward + step inside)
            metrics = phase2_perstep_step(denoiser, inner, nbatch, device, world_size, cfg, optimizer, scheduler)
