...
class InnerModelConfig:
    ...
    channels: List[int] # num feature map channels at each resolution level
    attn_depths: List[bool] # whether self-attention should be applied at each resolution level
    num_actions: int = 7 # dim of action vector


def get_inner_model_config(config_name: str) -> InnerModelConfig:
    if config_name == "RT1_UNet":
